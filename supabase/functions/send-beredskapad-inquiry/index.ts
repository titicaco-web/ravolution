...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { name, company, role, email, interest, message } = body ?? {};
    ...
    if (String(name).length > 200 || String(email).length > 320 || String(message).length > 4000) {
    ...
    const html = `
<h2>Beredskapad.se — New Acquisition Enquiry</h2>
...
<p style="color:#999;font-size:12px;margin-top:24px;">Sent via ravolution.se/en/beredskapad</p>
`;
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [RECIPIENT],
        reply_to: email,
        subject: `Beredskapad.se — ${safeInterest} — from ${safeName}${safeCompany ? ` (${safeCompany})` : ""}`,
        html,
      }),
    });
