...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const html = `
<h2>XportMatch.com — New Acquisition Enquiry</h2>
...
<p style="color:#999;font-size:12px;margin-top:24px;">Sent via ravolution.se/en/xportmatch</p>
`;
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [RECIPIENT],
        reply_to: email,
        subject: `XportMatch.com — ${safeInterest} — from ${safeName}${safeCompany ? ` (${safeCompany})` : ""}`,
        html,
      }),
    });
