...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const html = `
<h2>AlarmSole — New Strategic Enquiry</h2>
...
<p style="color:#999;font-size:12px;margin-top:24px;">Sent via ravolution.se/en/alarmsole</p>
`;
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [RECIPIENT],
        reply_to: email,
        subject: `AlarmSole — ${safeInterest} — from ${safeName}${safeCompany ? ` (${safeCompany})` : ""}`,
        html,
      }),
    });
